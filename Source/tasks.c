...
{
	dprintk("vTaskSwitchContext\n");
	if( uxSchedulerSuspended != ( unsigned portBASE_TYPE ) pdFALSE )
	{
		dprintk("vTaskSwitchContext: scheduler suspended\n");
	...
	{
		dprintk("vTaskSwitchContext: scheduler running\n");
		xYieldPending = pdFALSE;
		traceTASK_SWITCHED_OUT();

		#if ( configGENERATE_RUN_TIME_STATS == 1 )
		{
			dprintk("vTaskSwitchContext: generating runtime stats\n");
				#ifdef portALT_GET_RUN_TIME_COUNTER_VALUE
		...
		dprintk("vTaskSwitchContext: first check for stack overflow\n");
		taskFIRST_CHECK_FOR_STACK_OVERFLOW();
		dprintk("vTaskSwitchContext: second check for stack overflow\n");
		taskSECOND_CHECK_FOR_STACK_OVERFLOW();

		dprintk("vTaskSwitchContext: old task is 0x%x, old task stack is 0x%x\n", pxCurrentTCB, pxCurrentTCB->pxTopOfStack);
		taskSELECT_HIGHEST_PRIORITY_TASK();
		dprintk("vTaskSwitchContext: new task is 0x%x, new task stack is 0x%x\n", pxCurrentTCB, pxCurrentTCB->pxTopOfStack);
		...
		#if ( configUSE_NEWLIB_REENTRANT == 1 )
		{
			...
			dprintk("vTaskSwitchContext: setting impure ptr\n");
			_impure_ptr = &( pxCurrentTCB->xNewLib_reent );
		}
		#endif /* configUSE_NEWLIB_REENTRANT */
	}
	dprintk("vTaskSwitchContext done\n");
}
/*-----------------------------------------------------------*/
...
static portTASK_FUNCTION( prvIdleTask, pvParameters )
{
	...
	dprintk("Idle task started\n");

	for( ;; )
	{
		dprintk("Idle task iter\n");
...
tskTCB *pxNewTCB;
	...
	if( pxNewTCB != NULL )
	{
		...
		dprintk("pxStack for TCB 0x%x is 0x%x\n", pxNewTCB, pxNewTCB->pxStack);
		...
		{
			dprintk("Allocated task stack at 0x%x\n", pxNewTCB->pxStack);
